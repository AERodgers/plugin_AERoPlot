...
while keepGoing
    ...
    if sortByBrightness
        @unsortByBrightness: stdPaletteSize,
                       ... "stdPaletteVector$",
                       ... "stdPaletteName$"
    endif
    removeObject: table
    ...
    @writeVars: dataDir$, "f1f2Plot.var"
endwhile

# SCRIPT SPECIFIC PROCEDURES (use only global variables) #######################
# Variable Management
procedure defineVars

    # GET / CREATE UI VARIABLES
    # Create UI.Vars if it doesn't exist.
    if !fileReadable(dataDir$)
        createDirectory: dataDir$
    endif
    if !fileReadable("data/f1f2Plot.var")
        @createFPVars: "data/f1f2Plot.var"
    endif
    @readVars: dataDir$, "f1f2Plot.var"

    # Reset UI.Vars if it doesn't contain the variable fpVersion$ (pre v 1.2.1).
    if !variableExists("fpVersion$")
        @createFPVars: "data/f1f2Plot.var"
        @readVars: dataDir$, "f1f2Plot.var"
    endif

    # Reset UI.Vars if formatPlotter has recently been updated
    if fpVersion$ != curFPVersion$
        @createFPVars: "data/f1f2Plot.var"
        @readVars: dataDir$, "f1f2Plot.var"
    endif
...
procedure createFPVars: .address$
    writeFileLine: .address$, "variable", tab$, "value"
    appendFileLine: .address$, "fpVersion$", tab$, curFPVersion$
...
# UI form and data processing procedures.
procedure doInputUI

    beginPause: "Formant plotter: data table settings"
        sentence: "Table address or object number", x_tableID$
        optionMenu: "Table format", x_tableFormat
        ...
        sentence: "Main factor", x_outerFactor$
        boolean: "Include secondary factor", x_useInnerFactor
        sentence: "Secondary factor", x_innerFactor$
        sentence: "F2 Column", x_f2Col$
        sentence: "F1 Column", x_f1Col$
        comment: "Formant frequency units in table"
        optionMenu: "Input units", x_inputUnits
            option: "Hertz"
            option: "Bark"

        boolean: "Use tertiary filters", x_tertiaryFilters
...
# Plot parameter calculation and drawing procedures
...
procedure createSubTables
    for outerLevel to outerLevels
        ...
        curColVector$ = stdPaletteVector$[outerColour[outerLevel]]
        curColName$ = stdPaletteName$[outerColour[outerLevel]]
